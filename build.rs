...
fn build_cflags() -> String {
    if cfg!(feature = "custom-cflags") {
        ...
        // TODO: Different set of flags here?
        "-O3 -march=native -mtune=native -fPIC".into()
    } else {
        "".into()
    }
...
impl Conf {
    ...
    fn build_extern(&self) -> Result<()> {
        ...
        if let Ok(gmp_inc) = std::env::var("DEP_GMP_INCLUDE_DIR") {
            builder.include(gmp_inc);
        }

        builder.try_compile("extern")?;
...
impl Conf {
    ...
    fn bindgen(&self) -> Result<()> {
        ...
        let bindings = builder
            .clang_arg(if let Ok(gmp_inc) = std::env::var("DEP_GMP_INCLUDE_DIR") {
                format!("-I{}", gmp_inc)
            } else {
                "".into()
            })
